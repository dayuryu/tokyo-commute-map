...
最終整理日: 2026-05-22（i18n P0 + P1 AI reason 完了、area_features 中文版が小雪 paste 待ち）

---

## ⭐ 次に着手する候補（優先度別）

### 🥇 最優先（次 session で再開時の起点）
1. **area_features 中文版 merge（主人手工 work pending）** — 主人が小雪に
   `_handoff/zh_translation_for_xiaoyuki/` を渡して 47 batch の JSON 回収中。
   完了後 `python scripts/merge_area_features_responses.py --lang zh` →
   commit `public/data/area_features_zh.json` → push。残作業は主に主人手工。
2. **i18n P1 続行（凛 next code work 候補）** — Story 物語 3 章 / MapView popup /
   TimeSlider / TransferFilter / CorrectionReporter 翻訳。詳細は下記「🌐 多言語化 i18n」section。

### 🥈 次の大物候補（area_features merge + i18n P1 完了後、プロダクト方針で 1 つ選ぶ）
- **A: i18n P2 続行** — Metadata 多言語化 / sitemap hreflang / Legal / SEO landing
...
- **E: B 清单基建続行** — `lib/hooks/` 5 個抽出 / `components/` feature 分目錄 / `lib/api/*` zod schema
...
### P1 — 次要 user-facing（凛 next code work 候補）
- [ ] Story 物語 3 章翻訳（品牌叙事、凛が母語クオリティで初訳 → 主人レビュー）
