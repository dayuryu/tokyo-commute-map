...
- **D: ~~AI 推薦 funnel 計測（GA4 統合）~~ 実装済（2026-06-08）** — 残は運営側: GA4 property 作成 + `NEXT_PUBLIC_GA4_ID` を Vercel / `.env.local` に設定（下記「計測・解析」参照）
...
## 🌐 多言語化 i18n（ja / en / zh 3 locale 上線済 — 残は P2 SEO・法務系）
...
### P2 — SEO / 法務 / 国際化深層（残）
- [ ] sitemap.ts hreflang + `/zh`・`/en` URL 出力（現状 sitemap は ja のみ）
- [ ] Legal 4-5 頁中文版・英文版（合規要件あり、機械翻訳禁止、人手翻訳のみ）
- [ ] `/to/[slug]` SEO landing 30 頁の中文版・英文版（ja 版と同じ外部 LLM workflow を言語切替で再利用、stats は ja と共通）
...
- [x] **Google Analytics 4 統合 + アフィリエイトボタンのクリック計測**（2026-06-08 コード実装済）
  ...
  - [ ] **残（運営側）**: GA4 property 作成 → measurement ID（G-XXXX）を取得 →
    Vercel 環境変数 + `.env.local` に `NEXT_PUBLIC_GA4_ID` を設定（未設定の間は何も送信されない）
  - [ ] **残（運営側）**: プライバシーポリシーに Google Analytics 利用の記載が必要か確認
    （現行文面が「解析ツール」を既にカバーしていれば不要）
