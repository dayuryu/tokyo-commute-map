...
最終整理日: 2026-06-10 夜（二拠点通勤上線 / ryugaku v2.1 / PR 表記撤去を反映）
...
2. **URL 索引リクエスト**（運営側、1 日 10 件 × 3〜4 日） — **手順と全 URL リストは `docs/search-console-indexing.md`（2026-06-11 作成、コピペで消化可能）**。
   旧 A 案と Rich Results 検証を統合：FAQPage / BreadcrumbList の構造化データは 2026-06-11 にコード側で全 30 駅機械検証済（全 PASS）。
   Rich Results Test はログイン必須化されたため目視確認のみ運営側（任意、1 分）
...
- **A: ~~URL 索引リクエスト 30 駅~~ 最優先 2. に統合（2026-06-11、`docs/search-console-indexing.md` 参照）**
...
- `a65a9a6` **v2.1 称号主役化** — 自創 4 字母代号（先験認知ゼロ）を最大表示から降格、中文称号が視覚主役に。
  代号は副句下の「PCRI · 全网约 X%」小字行（隠し型は非表示）、四維 slider 両極に字母併記で代号の意味を自解释。
  O 極「远郊妥协」→「远郊自在」（8 極中唯一の負面表現、道德対称原則違反だった）。結果頁 / 分享卡 / OG の 3 面同期。
  **OG は s-maxage=1y immutable のため版式変更時は page.tsx の og:image URL の `&v=` を上げて cache bust すること**（現 v=2）
- `dde6e39` **隠し型 otome / nishikasai 到達可能化**（2026-06-11） — 問題は 20+4 のまま増やさず既存回答の組合せで触発:
  中野乙女 = x1 満点 + 強追夢 + 死守都心（geo≥4、谷子聖地常駐）/ 圣地巡礼者は非強都心側に変更（互斥）。
  西葛西咖喱党 = 彩蛋 4 問全部「看情况」+ 四軸全て |raw|≤2（随遇而安）— 全部中立のユーザーは
  名ばかり PCRI でなく稀有咖喱党に。20 万乱数採样: 乙女 0.84% / 咖喱党 0.019%

### 残タスク
- [ ] **真機検証**（運営側、最優先 section 参照） — 通し流れ + 微信共有
- [x] ~~隠し型 otome / nishikasai のトリガー問題追加~~ 完了（2026-06-11、`dde6e39`、上記参照）。
  上線後、咖喱党の実際の命中率（彩蛋全中立の比率次第）を見て主軸閾値 ±2 を要調整かも
...
## 🗺️ 二拠点通勤（2 つ目の目的地） — MVP 上線済（2026-06-10）

> 夫婦 / カップルの双通勤シーン向け。地図上で「両方の目的地に通える駅」だけを
> max(A, B)（悪い方）で着色表示する。全 1831 駅が第 2 目的地に指定可能
> （30 固定駅は預計算 `min_to_<slug>`、それ以外は client Dijkstra `secondCommutesAtom`）。

### 上線済（merge / commit 時系列、全て 2026-06-10）
- `72bc605` **MVP** — `setSecondDestinationAtom`（domain.ts 流の唯一書き込み口、null=解除、localStorage `tcm.destination2.v1`）/
  MapView は min_to_pair 実行時注入で cluster も pair 染色 / 2 本目の赤ピン / StationDrawer に 2 行目の所要時間。
  副産物: MapView データ effect に `destInfoReady` 依存を追加し localStorage 復元 race の既存 bug も根治
- `a84671b` **ゴーストチップ** — 裸の「＋」を破線チップ「＋ 目的地を追加」に（mobile は短縮「＋ 目的地」）
- `43ab8cc` **残留 fix 3 件 + 不変量** — ① 検索 close 時の blur（iOS キーボード残留）② チップ弾入残影（常時 mount + collapse 遷移）
  ③ fixed 駅検索選択後に検索欄が残る既存 bug ④ domain 層で primary == second の重複ペアを双方向ガード

### 二期候補（未着手）
- [ ] AI 推薦 `/api/recommend` の双目的地対応（cache 組合せ ~435 維、hit 率低下は許容範囲と試算済）
- [ ] 合成関数の加重版（通勤日数で weight、UI 1 問追加）
- [ ] pair 落地頁 SEO（30×30/2 ≈ 435 組合せの SSG、流量戦略次第）
...
> 📌 **2026-06-10: 「PR」表記を撤去**（`840b1d9`、ユーザー決定「短期は広告接続せず SEO / 流量優先」）。
> 未契約 = 無報酬の素リンクに PR 表記は事実不一致のため、PR ラベル / `rel="sponsored"` /
> 「広告について」link を撤去し、`/legal/ads` 冒頭を「現在掲載していません」に改訂。
> **ASP 契約時は `affiliate-compliance.md` §3-1 の注記に従い全て復元すること**（ステマ規制）。
...
### Affiliate 深化
- [x] **HOME'S / CHINTAI 駅 deep link** ✅ 完了（2026-06-10、`840b1d9`）
  - 反向工程の結果、駅マップ不要の GET パラメータで実現（全 1831 駅対応）:
    HOME'S `/chintai/list/?cond[freeword]=<駅名>&cond[fwtype]=2`（実地検証: 駅周辺に絞られる）/
    CHINTAI `/direct/?dir_word=<駅名>`（トップ検索フォームと同経路）。
    併せて駅名 normalize に消歧括弧除去を追加（田町(東京) 等が SUUMO マップにもヒットするように）
...
- [x] **SEO 内部リンク網 + /to hub**（2026-06-11 完了）— 準孤児頁問題の根治。
  - 問題: `/to/*` 30 頁と `/ryugaku` への内部リンクが全站に 0 本（sitemap のみ）。
    トップは map app で SSR HTML に `<a>` が 1 本も無かった（overlay 全条件付き描画のため）
  - ✅ `/to` hub 新設（3 locale SSG、30 駅リスト + 統計、CollectionPage/ItemList JSON-LD、sitemap + hreflang）
  - ✅ トップ SSR HTML に nav（`welcomeOpen===null` の hydration 前のみ描画 = 素 HTML 用、隠しリンク無し）
  - ✅ WelcomeOverlay 底部に可視リンク（通勤先ガイド + zh は留学テストも）
  - ✅ HeaderMenu に通勤先ガイド / 留学テスト(zh) 項目（dropdown は常駐 DOM で crawlable）
  - ✅ `/to/[slug]` に BreadcrumbList JSON-LD + footer に hub 回链。loadStats は `lib/destination-stats.ts` に共通化
  - ✅ 構造化データ機械検証: 線上 + ローカル全 30 駅 FAQPage/BreadcrumbList 全 PASS
  - 不做と判断: pair 落地頁 435 組合せの量産（thin content リスク + 既存 30 頁の索引実績を見てから）
...
| 2026-06-10 (夜) | **① 二拠点通勤 MVP 上線**（`72bc605` + ゴーストチップ `a84671b` + 残留 fix / 不変量 `43ab8cc`）— 第 2 目的地を max(A,B) 合成着色、全 1831 駅対応、専用 section 新設。**② /ryugaku v2.1**（`a65a9a6`）— 称号主役化（代号は小字に降格 + slider 字母併記）+ O 極「远郊自在」改名、3 表示面同期、OG cache bust `&v=2`。**③ PR 表記撤去 + HOME'S / CHINTAI 駅 deep link**（`840b1d9`）— 未契約のため広告表記を事実に合わせ撤去（復元手順は compliance §3-1）、deep link は GET パラメータで駅マップ不要。全て Vercel 部署済 |
