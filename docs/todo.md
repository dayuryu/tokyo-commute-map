...
- `2f66fdd` **朋友圈分享按钮**（2026-06-11） — 網頁は朋友圏を直接起動できない（JS-SDK は認証服務号が必要）ため誘導方式:
  微信内は「右上角 ···」誘導蒙層 + `?a=` を replaceState で書き戻し + document.title 個性化（分享リンクが結果持ち、受け手は SSR 個性化 OG）/
  微信外はリンク複製 + 微信で貼る誘導（保存分享卡で発図の提示付き）。Edge headless で両 UA 検証済

### 残タスク
- [ ] **真機検証**（運営側、最優先 section 参照） — 通し流れ + 微信共有。朋友圈按钮は追加で 2 点確認:
  ① 旧 iOS 微信は replaceState 後でなく着地 URL を分享する怪癖があった — 分享後に受け手側で結果が開くか
  ② JS-SDK 無しの分享卡縮図は微信が自前抓取（OG 型色卡が使われる保証なし）— 不満なら公众号 + JS-SDK 路線を検討
